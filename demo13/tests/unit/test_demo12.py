...
from demo12.src.serializers import CsvSerializer, JsonSerializer, XmlSerializer
...
class TestDemo12:
    ...
    def xml_serialize_error(self):
        return XmlSerializeError(
            "XML serialization failed", "Missing closing tag", "<user>"
        )
    ...
    @pytest.fixture
    def app_with_xml_serializer(
        self, database_repository, xml_serializer, event_service_client
    ):
        return EventApplication(
            database_repository, xml_serializer, event_service_client
        )

    @pytest.fixture
    def app_with_json_serializer(
        self, database_repository, json_serializer, event_service_client
    ):
        return EventApplication(
            database_repository, json_serializer, event_service_client
        )

    @pytest.fixture
    def app_with_csv_serializer(
        self, database_repository, csv_serializer, event_service_client
    ):
        return EventApplication(
            database_repository, csv_serializer, event_service_client
        )
    ...
    def error_handler_factory(self):
        ...
        error_handler_factory.register_handler(
            SerializeError, handle_serialize_error_text
        )
        error_handler_factory.register_handler(
    ...
    def test_json_serializer_with_falsy_data(self, json_serializer):
        with pytest.raises(JsonSerializeError) as exc_info:
            json_serializer.serialize(
                None
            )  # Or other falsy value expected to trigger the error
    ...
    def test_app_using_xml_serializer_to_handle_xml_serialize_error_text(
        self, caplog, app_with_xml_serializer: EventApplication, error_handler_factory
    ):
        caplog.set_level(
            logging.ERROR
        ...
        assert (
            "XML Serialization error occurred" in caplog.text
    ...
    def test_app_using_json_serializer_to_handle_json_serialize_error_text(
        self, caplog, app_with_json_serializer: EventApplication, error_handler_factory
    ):
        caplog.set_level(
            logging.ERROR
        )  # Set the logging level to capture ERROR messages
        ...
        assert log_record.levelname == "ERROR"  # Ensure the log level is ERROR
        assert (
            "JSON Serialization error occurred" in caplog.text
        )  # Check the message content
        assert "JSON serialization failed" in caplog.text  # Check the message content
        assert "Detail:" in log_record.message  # Check for detail information

    def test_app_using_csv_serializer_to_handle_csv_serialize_error_text(
        self, caplog, app_with_csv_serializer: EventApplication, error_handler_factory
    ):
        caplog.set_level(
            logging.ERROR
        )  # Set the logging level to capture ERROR messages
        ...
        assert log_record.levelname == "ERROR"  # Ensure the log level is ERROR
        assert (
            "CSV Serialization error occurred" in caplog.text
        )  # Check the message content
        assert "CSV serialization failed" in caplog.text  # Check the message content
        assert "Detail:" in log_record.message  # Check for detail information

    def test_app_using_csv_serializer_to_handle_csv_serialize_error_json(
        self, caplog, app_with_csv_serializer: EventApplication, error_handler_factory
    ):
        caplog.set_level(logging.ERROR)
        ...
        error_handler_factory.register_handler(
            CsvSerializeError, handle_csv_serialize_error_json
        )
        error_handler = error_handler_factory.get_handler(e.type)
        error_handler(e.value)
    ...
    def test_app_using_xml_serializer_to_handle_csv_serialize_error_json(
        self, caplog, app_with_xml_serializer: EventApplication, error_handler_factory
    ):
        caplog.set_level(logging.ERROR)
        ...
        error_handler_factory.register_handler(
            XmlSerializeError, handle_xml_serialize_error_json
        )
        error_handler = error_handler_factory.get_handler(e.type)
        error_handler(e.value)
